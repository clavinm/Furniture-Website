...
function reset_link($get_email,$token)
{
    $mail = new PHPMailer(true);
                    ...
                    <a href='http://localhost/furniture-shop-main/pass-change.php?token=$token&email=$get_email'> Click Me </a>
                    ";
...
if(isset($_POST['submit']))
{
    ...
    $token = md5(rand());
    $check_email = "SELECT cust_email FROM customer WHERE cust_email= '$email' LIMIT 1";
    $check_email_run = mysqli_query($con, $check_email);
    if(mysqli_num_rows($check_email_run) > 0)
    {
        $row= mysqli_fetch_array($check_email_run);
         $get_email = $row['cust_email'];
    
        $update_token = "UPDATE customer SET verify_token= '$token' WHERE cust_email='$get_email' LIMIT 1";
    ...
    if(!empty($token))
    {
        if(!empty($email) && !empty($new_password) && !empty($confirm_password))
        {
            ...
            if(mysqli_num_rows($check_token_run) > 0)
            {
                ...
                {
                    $update_password = "UPDATE customer SET cust_pass='$new_password' WHERE verify_token='$token' LIMIT 1";
                    $update_password_run = mysqli_query($con, $update_password);
                    if($update_password_run)
                    {
                        $new_token = md5(rand())."Souza"; 
                        $update_new_token = "UPDATE customer SET verify_token='$new_token' WHERE verify_token='$token' LIMIT 1";
